...
const CarbonTrackerSection = ({
  isMNC,
  onStatsUpdate
}: CarbonTrackerSectionProps) => {
  const {
    user
  } = useAuth();
  const {
  ...
  const fetchRecords = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('carbon_tracking').select('*').eq('user_id', user.id).order('created_at', {
      ascending: false
    });
    if (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: signedData,
        error: signedError
      } = await supabase.storage.from('bills').createSignedUrl(fileName, 31536000); // 1 year
      ...
      const {
        data,
        error: funcError
      } = await supabase.functions.invoke('process-bill', {
        body: {
          billUrl,
          monthYear,
          state
      ...
      const {
        data: currentPoints
      } = await supabase.from('user_points').select('total_points, carbon_saved').eq('user_id', user.id).single();
  ...
  return <div className="space-y-6">
      <Card className="carbon-card">
        <CardHeader>
          ...
          <CardDescription>
            {isMNC ? "Track monthly corporate carbon emissions and earn sustainability points" : "Track your carbon footprint and earn points for lower emissions"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="monthYear" type="month" value={monthYear} onChange={e => setMonthYear(e.target.value)} required />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={state} onValueChange={setState}>
                ...
                <SelectContent className="bg-background">
                  {INDIAN_STATES.map(stateName => <SelectItem key={stateName} value={stateName}>
                      {stateName}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Input id="billFile" type="file" accept=".pdf,image/*" onChange={handleFileChange} required />
              <p className="text-xs text-muted-foreground">Upload your bill and we'll calculate carbon footprint.</p>
            </div>
      ...
      <Card className="carbon-card">
        ...
        <CardContent>
          {records.length === 0 ? <p className="text-center text-muted-foreground py-8">No records yet</p> : <div className="space-y-4">
              {records.map(record => <div key={record.id} className="border border-border rounded-lg p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {record.state && <p className="text-xs text-muted-foreground">
                          📍 {record.state}
                        </p>}
                    </div>
